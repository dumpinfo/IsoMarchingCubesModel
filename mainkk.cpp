	...
	for(i = 30; i<330; i++)
	{
       for( j= 30; j < 330; j++ )
		   ...
		   for( t= 30 ; t < 330; t++ )
		   {
			   ...
			   //y = 128 + t;
              int a1,a2,a3;
			  a1 = (i-30)%gridSize; a2 = (j-30)%gridSize; a3 = (t-30)%gridSize;
			  *get_voxl_color(VolDat,i,j,t) = 8000;
			  
			  if((a1>gridThick&&a1<(gridSize - gridThick))&&(a2>gridThick&&a2<(gridSize - gridThick)))
 			  *get_voxl_color(VolDat,i,j,t) = 0;
		  
		      if((a2>gridThick&&a2<(gridSize - gridThick))&&(a3>gridThick&&a3<(gridSize - gridThick)))
			  *get_voxl_color(VolDat,i,j,t) = 0;
		  
		      if((a1>gridThick&&a1<(gridSize - gridThick))&&(a3>gridThick&&a3<(gridSize - gridThick)))
			  *get_voxl_color(VolDat,i,j,t) = 0;
